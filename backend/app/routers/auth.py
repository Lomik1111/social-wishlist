...
index d4768f472e9d20944893b1553a399be90fc5ffe8..5770e511b4965a7652f119e27a8b44a500f5eabb 100644
...
+        raise HTTPException(
+            status_code=503,
+            detail="Google OAuth transport dependency is missing on server",
+        ) from exc
+
+    try:
...
+
+    result = await db.execute(select(User).where(or_(User.email == email, User.oauth_id == google_sub)))
...
@@ -46,53 +159,51 @@ async def login(data: UserLogin, db: AsyncSession = Depends(get_db)):
...
-    )
+    result = await db.execute(select(RefreshToken).where(RefreshToken.token_hash == token_h, RefreshToken.revoked == False))
